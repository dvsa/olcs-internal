...
return [
    'version' => ($isProduction ? [] : $versionInfo),
    ...
    'Logger' => [
        'writers' => [
            'full' => [
                'options' => [
                    'stream' => '/var/log/dvsa/olcs-iuweb/iuweb.log'
                ],
            ]
        ]
    ],
    ...
    'algo' => '%cache_encryption_algo%',
        ...
        'secrets' => [
    'node' => '%= scope().call_function('lookup',['cache_encryption_secret_iu']) %>',
            'shared' => '<%= scope().call_function('lookup',['cache_encryption_secret_shared']) %>',
        ],
    ],

    'query_cache' => [
    // whether the cqrs cache is enabled
    'enabled' => <%= scope().call_function('lookup',['cqrs_cache_enabled']) %>,
    ...
    'auth' => [
    'user_unique_id_salt' => '<%= scope().call_function('lookup',['user_unique_id_salt']) %>',
        'realm' => 'internal',

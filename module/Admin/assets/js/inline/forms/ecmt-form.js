OLCS.ready(function () {
    ...
    function validatePermitsRequired(euro5, euro6, clickEvent) {
        ...
        if (
            (euro5.val() !== "" && parseInt(euro5.val(), 10) < 1) ||
            (!$.isNumeric(euro5.val()) && euro5.val().length > 0)
        ) {
            wrapError(euro5, "Must be a positive integer if value is provided.", clickEvent);
        }
        if (
            euro6.val() !== "" && parseInt(euro6.val(), 10) < 1 ||
            (!$.isNumeric(euro6.val()) && euro6.val().length > 0)
        ) {
            wrapError(euro6, "Must be a positive integer if value is provided.", clickEvent);
        }
